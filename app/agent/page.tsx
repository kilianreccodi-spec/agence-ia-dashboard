    ...
    <div style={{ display: "flex", height: "100vh", background: c.bg, color: c.text, fontFamily: "DM Sans, sans-serif", overflow: "hidden" }}>
      ...
      <aside style={{ width: 200, background: c.sidebar, borderRight: `1px solid ${c.border}`, display: "flex", flexDirection: "column", flexShrink: 0 }}>
        ...
        <nav style={{ padding: "14px 10px", display: "flex", flexDirection: "column", gap: 3 }}>
          ...
          {["Dashboard", "Agents", "Task Board", "Organigramme", "Offre"].map((item) => (
            <a key={item} href="/" style={{ fontSize: 13, padding: "8px 12px", borderRadius: 6, color: c.textMuted, textDecoration: "none", display: "block", borderLeft: "2px solid transparent" }}>{item}</a>
          ))}
          <div style={{ fontSize: 13, padding: "8px 12px", borderRadius: 6, color: c.text, borderLeft: `2px solid ${c.accent}`, background: c.hover, fontWeight: 500, marginTop: 4 }}>Démo agent</div>
        </nav>
      </aside>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ padding: "16px 24px", borderBottom: `1px solid ${c.border}`, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ marginLeft: "auto", fontSize: 10, color: c.textMuted, background: c.card, border: `1px solid ${c.border}`, padding: "4px 10px", borderRadius: 4 }}>DÉMO INTERACTIVE · OPTIMA FLOW</div>
        </div>
        ...
        <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, padding: 20, overflow: "hidden" }}>
          ...
          <div style={{ background: c.card, border: `1px solid ${c.border}`, borderRadius: 12, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ padding: "12px 16px", borderBottom: `1px solid ${c.border}`, fontSize: 12, fontWeight: 500, color: c.textSub }}>Conversation avec Alex</div>
            ...
            {showSuggests && (
              <div style={{ padding: "8px 14px", display: "flex", flexWrap: "wrap", gap: 5 }}>
                {SUGGESTIONS.map((s) => (
                  <button key={s} onClick={() => send(s)} style={{ fontSize: 11, padding: "5px 10px", border: `0.5px solid ${c.border}`, borderRadius: 20, background: c.cardAlt, color: c.textMuted, cursor: "pointer", fontFamily: "inherit" }}>{s}</button>
                ))}
              </div>
            )}
            <div style={{ padding: "10px 14px", borderTop: `1px solid ${c.border}`, display: "flex", gap: 8 }}>
              <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder="Écrivez à Alex..." style={{ flex: 1, padding: "8px 12px", fontSize: 12, border: `1px solid ${c.border}`, borderRadius: 8, background: c.cardAlt, color: c.text, outline: "none", fontFamily: "inherit" }} />
          ...
          <div style={{ background: c.card, border: `1px solid ${c.border}`, borderRadius: 12, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
              ...
              {[
                ...
                <div key={i} style={{ background: c.cardAlt, border: `1px solid ${c.border}`, borderRadius: 8, padding: "10px 12px", marginBottom: 6 }}>
              ...
              {EMAILS.map((e, i) => (
                <div key={i} style={{ borderLeft: `2px solid ${c.green}`, background: c.cardAlt, borderRadius: "0 8px 8px 0", padding: "8px 12px", marginBottom: 6 }}>
